...
func TestSimple(t *testing.T) {
	...
	w1 := wapitesting.NewWorkflow(group, version, "mydag", "mynamespace", nil)
	runTest(t, w1, gvk)
}

func runTest(t *testing.T, w1 *wapi.Workflow, gvk *unversioned.GroupVersionKind) {
	u, err := WorkflowToUnstructured(w1, gvk)
	if err != nil {
		t.Fatal(err)
	}
	w2, err := UnstructuredToWorkflow(u)
	if err != nil {
		t.Fatal(err)
	}

	if !api.Semantic.DeepEqual(w1, w2) {
		file1, err := wapitesting.DumpWorkflowInTmpFile(w1)
		if err != nil {
			t.Fatalf("diff found but unable to dump workflow: %v", err)
		}
		file2, err := wapitesting.DumpWorkflowInTmpFile(w2)
		if err != nil {
			t.Fatalf("diff found but unable to dump workflow: %v", err)
		}
		t.Fatalf("diff found dumped files %s %s\n ", file1, file2)
	}
}
...
func TestEncodeDecodeFuzzer(t *testing.T) {
	group := "example.com"
	version := "v1"
	kind := "Workflow"
	...
	f := fuzz.New().Funcs(
		func(j *runtime.TypeMeta, c fuzz.Continue) {
			j.APIVersion = fmt.Sprintf("%s/%s", gvk.Group, gvk.Version)
			j.Kind = gvk.Kind
		},
		...
		func(s *api.PodSpec, c fuzz.Continue) {
			...
			s.InitContainers = []api.Container{} // @sdminonne TODO: remove this
		},
